<?php class mMarca_wh {
	...
	public function lista() {
		$sql = "SELECT * FROM vw_tbl_marca_wh WHERE marca_borrado=0 ORDER BY marca_ide DESC";
		return Enlace::sql($sql,'',3,'');
	}

	public function poride($ide) {
		$sql = "SELECT * FROM vw_tbl_marca_wh WHERE marca_ide=?";
		$datos = array($ide);
		return Enlace::sql($sql,$datos,3,'');
	}

	public function insert() {
		$sql = "SELECT sf_marca_wh(?,?,?,?) AS res";
