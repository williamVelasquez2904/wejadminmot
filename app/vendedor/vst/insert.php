...
<div class="alert alert-info">
	<i class="fa fa-exclamation-triangle fa-3x pull-left red"></i> Por favor rellene el siguiente formulario para agregar un vendedor.
</div>
<form action="" class="op1-vende">
	<div class="msj-vende" id="errores"></div>
	<!-- Datos del cliente ############### -->
	<fieldset><legend>Datos vendedor</legend>	
		<div class="form-group col-sm-4">
			<label for="" class="label control-label col-sm-12 bolder">Tipo:</label>
			<div class="col-sm-12" id="tipovende">
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mvendedor-insert',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						load('vst-vendedor-datos.personales','vende_ide='+data.trim(),'.perfil');
					} else {
						var data1=data.split(' - ');
						if (data1[0]=='vendedor se encuentra Inactivo') {
							
							

							if(confirm('vendedor se encuentra Inactivo, ¿Desea Reactivarlo?')==true) {
								$.post('prc-mvendedor-reactivarlo','ide='+data1[1],function(data2){
									if(!isNaN(data2)) {
										alert('El vendedor fue Reactivado satisfactoriamente')
										load('vst-vendedor-datos.personales','vende_ide='+data1[1],'.perfil');
									} else {
							...
							alerta('.msj-vende','danger',data1[0]);
						}
			...
			tag_input.tag(
			  {
				...
				source: [<?php foreach($mvendedor->lista() as $r): ?> "<?php echo $r->vende_ide.' - '.$r->vende_tipo.$r->vende_cedrif.' - '.$r->vende_razonsocial.' '.$r->vende_alias; ?>", <?php endforeach; ?>]
			  }
			);
		}
		else {
