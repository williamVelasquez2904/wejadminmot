	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mcompra-insert_venta',$(formulario).serialize(),function(data){
					alerta('.msj','danger',data)
					if(!isNaN(data)) {
						alerta('.msj','success','Registro modificado correctamente');
						modal('vst-compracom-detacompra','orden_ide=<?php echo $orden_ide ?>&compra_encab_ide=<?php echo $encab_ide ?>');
					} else {
						alerta('.msj','danger',data)
					}
				})
