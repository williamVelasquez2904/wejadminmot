...
<?php foreach($mimpuesto->poride($ide) as $r): ?>
	<form action="" class="op2 form-horizontal">
		<?php echo $fn->modalHeader('Editar Impuesto') ?>
		<div class="modal-body">
			<div class="msj"></div>
			<div class="form-group">
				<label for="" class="control-label col-sm-3 bolder">Porcentaje o Tasa de Impuesto:</label>
				<div class="col-sm-9">
					<input type="text" class="form-control" name="des" value="<?php echo $r->impuesto_porcent ?>">
				</div>
			</div>
		</div>
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mimpuesto-update',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
