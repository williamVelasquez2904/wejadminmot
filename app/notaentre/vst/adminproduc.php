...
<form action="" class="cargarproduc">
	<div class="form-group">
		<label for="" class="label control-label col-sm-12 bolder">Buscar Producto o Servicio para cargar a la nota de entrega:</label>
		<div class="col-sm-12">
			<div class="table-responsive">
				<table class="table table-bordered">
					<tr>
						...
						<td width="45%">
							<?php if ($buscaractivo==1) { $_SESSION['s_notaentre']=$noen;?>
							<div class="col-sm-9">
			...
			submitHandler: function (form) {
				...
				$.post('prc-cproducto-buscarfactu',$(formulario).serialize(),function(data){
					if(data.trim()=='no') {
						...
						load('vst-notaentre-adminproduc','buscaractivo=1&noen=<?php echo $_SESSION['s_notaentre']; ?>','.notaentre');
					} else {
						load('vst-notaentre-adminproduc','produc_ide='+data.trim()+'&buscaractivo=0','.notaentre');
						load('vst-notaentre-detanoen','','.detanoen');
					}
				})
				<?php } else {?>
				$.post('prc-mnotaentre-inserttemp',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						load('vst-notaentre-adminproduc','buscaractivo=1&noen=<?php echo $s_notaentre; ?>','.notaentre');
						load('vst-notaentre-detanoen','','.detanoen');
					} else {
