		...
		<fieldset><legend>Datos Personales del Cliente</legend>	
			<div class="form-group col-sm-3">
				...
				<div class="col-sm-10" id="tipcli">
					<select class="form-control chosen" title="Agregar" name="" id="tipcli" onchange="load('vst-tipclien-select','tipcli_ide='+this.value,'#tipcli');">
						<option value=""></option>
			...
			<div class="form-actions">
				<div class="btn-group pull-right">
					...
					<button type="button" class="btn btn-danger btn-sm pull-right" onclick="load('vst-notaentre-datos.personales','clien_ide=<?php echo $clien_ide ?>','.perfil')"><span class="i fa fa-times"></span> Cancelar</button>
				</div>
			</div>
		</fieldset>
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mnotaentre-updatecli',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						alert('Cambios guardados correctamente');
						load('vst-notaentre-datos.personales','clien_ide=<?php echo $clien_ide ?>','.perfil');
					} else {
