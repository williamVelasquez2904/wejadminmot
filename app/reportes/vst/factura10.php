...
'</table>';
if ($factura_rows[0]->fact_sub_totalexe<=2000000) {$mosiva=9;}
else {$mosiva=7;}
...
$cuerpo =''.
  '<div id="contenido">'.
    ...
    foreach($facturadeta_rows as $detafact) {
      if ($detafact->produc_impuesto==12) { $impuesto12=$impuesto12+$detafact->Impuesto10; 
        $baseimponible=$baseimponible+$detafact->factdeta_base; 
        $mosivadet=$mosiva;
      } elseif ($detafact->produc_impuesto==8) { $impuesto8=$impuesto8+$detafact->Impuesto; 
        $baseimponible=$baseimponible+$detafact->factdeta_base; 
        $mosivadet=$detafact->produc_impuesto;
      } elseif ($detafact->produc_impuesto==0) { $impuesto0=$impuesto0+$detafact->Impuesto; 
        $exento=$exento+$detafact->factdeta_base; 
        $mosivadet=$detafact->produc_impuesto;
      }
      ...
      '<tr>'.
        ...
        '<td width="15%" align="right">'.number_format(($detafact->factdeta_base/$detafact->factdeta_cant), 2, ',', '.').'</td>'.
        '<td width="15%" align="right"><b>'.number_format($detafact->factdeta_base, 2, ',', '.').'</b></td>'.
        '<td width="10%" align="right"><b>'.$mosivadet.'%'.'</b></td>'.
      '</tr>';
    }
    foreach($facturadeta_rows1 as $detafact1) {
      if ($detafact1->produc_impuesto==12) { $impuesto12=$impuesto12+$detafact1->Impuesto10; 
        $baseimponible=$baseimponible+$detafact1->factdeta_base; }
      ...
      elseif ($detafact1->produc_impuesto==0) { $impuesto0=$impuesto0+$detafact1->Impuesto; 
        $exento=$exento+$detafact1->factdeta_base; }
      $cuerpo .= ''.
      '<tr>'.
        ...
        '<td align="right"><b>'.number_format($detafact1->factdeta_base, 2, ',', '.').'</b></td>'.
