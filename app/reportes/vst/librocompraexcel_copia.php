...
<?php
		...
		$objPHPExcel->getProperties()->setCreator("Codedrinks") //Autor
							 ...
							 ->setDescription("Libro de Compras")
							 ->setKeywords("Libro de Compras")
							 ->setCategory("Reporte excel");

		$tituloReporte = "Libro de Compras ".$rowt[0]->empresa_nombre;
		$subtituloReporte = "Del ".date('d-m-Y',strtotime($f_ini))." al ".date('d-m-Y',strtotime($f_fin));
		$titulosColumnas = array('FECHA', 'FACTURA', 'PROVEEDOR', 'RIF', 'BASE IMPONIBLE', 'ALICUOTA', 'BsF. IVA', 'TOTAL', 'STATUS');
		...
		$i = 4;
		foreach($row as $r):
			if ($r->compra_anulada==0) { $anula="Activa"; } else { $anula="Anulada"; }
			$objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i,  date('d-m-Y',strtotime($r->compra_fecha)))
		            ->setCellValue('B'.$i,  $r->compra_ide_tienda)
            		...
            		->setCellValue('E'.$i,  number_format($r->compra_sub_totalexe, 2, ',', '.'))
            		->setCellValue('F'.$i,  '12%')
            		->setCellValue('G'.$i,  number_format($r->compra_sub_totaliva, 2, ',', '.'))
            		->setCellValue('H'.$i,  number_format($r->compra_total_general, 2, ',', '.'))
		...
		$objPHPExcel->getActiveSheet()->setTitle('Libro de Compras');
		...
		header("Content-Disposition: attachment; filename=Libro_de_Compra.xlsx"); 
		header("Expires: 0");
