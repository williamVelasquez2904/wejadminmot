	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-cfacturacion-buscar',$(formulario).serialize(),function(data){
					if(data.trim()=='no') {
						load('vst-facturacion-insert','','.perfil');
					} else {
						load('vst-facturacion-datos.personales','clien_ide='+data.trim(),'.perfil');
						$.post('prc-mfacturacion-insertfacttemp',0,function(data1){
							if(!isNaN(data1)) {
								load('vst-facturacion-adminproduc','buscaractivo=1&fact='+data1.trim(),'.factura');
							}
						})
					}
				})
