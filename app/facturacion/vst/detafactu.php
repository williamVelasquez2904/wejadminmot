...
<?php $rowdf2 = $mfacturacion->listadetafactura2($_SESSION['s_factura']); ?>
<?php $rowdf3 = $mfacturacion->listadetafactura3($_SESSION['s_factura']); ?>
<?php $rowdft = $mfacturacion->totalfactura($_SESSION['s_factura']); ?>
			...
			<tbody>
				<?php foreach($rowdf2 as $rdf2): 
					...
					<tr>
						...
						<td>
							<div class="btn-group">
								<button class="btn btn-success btn-xs" title="Actualizar" onclick="modal('vst-facturacion-updatedetafac','ide=<?php echo $rdf2->factdetatemp_ide; ?>')">
									<i class="fa fa-edit"></i>
								</button>
								<button class="btn btn-danger btn-xs" title="Borrar" onclick="modal('vst-facturacion-deletedetafac','ide=<?php echo $rdf2->factdetatemp_ide; ?>')">
				...
				<?php foreach($rowdf3 as $rdf3):  
					...
					<tr>
						<td><?php echo $rdf3->factdetatemp_produc .' - '.$rdf3->produc_codigo.' - '.$rdf3->produc_descrip.' - '.$rdf3->detaprod_imei.' - '.$rdf3->detaprod_serial; ?></td>
						<td align="center"><?php echo number_format($rdf3->factdetatemp_cant,2,",",".").' '.$unidad_med; ?></td>
						<td align="right"><?php echo number_format($rdf3->factdetatemp_precio,2,",","."); ?></td>
						<td align="right"><?php echo number_format($rdf3->factdetatemp_monto,2,",","."); ?></td>
						<td>
							<div class="btn-group">
								<button class="btn btn-success btn-xs" title="Actualizar" onclick="modal('vst-facturacion-updatedetafac','ide=<?php echo $rdf3->factdetatemp_ide; ?>')">
									<i class="fa fa-edit"></i>
								</button>
								<button class="btn btn-danger btn-xs" title="Borrar" onclick="modal('vst-facturacion-deletedetafac','ide=<?php echo $rdf3->factdetatemp_ide; ?>')">
									<i class="fa fa-trash"></i>
								</button>
							</div>
						</td>
					...
					<tr>
						<td width="53%">
							<table align="center">
								...
								<tr align="right">
									<th>Efectivo:</th><td><font size="2"><input type="number" min="0" step="0,01" class="form-control" name="efecti" value="<?php echo number_format($rowdft[0]->Total,2,".",""); ?>"></font></td>
									<th></th><td><font size="2"></font></td>
									<th></th><td><font size="2"></font></td>
								</tr>
								<tr align="right">
						...
						<td align="center">
							<div class="btn-group">
								...
								<br><br>
								<a href="?var=<?php echo base64_encode(7) ?>">
	...
	$(function(){
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mfacturacion-insertfact',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						alert("Factura Procesada Exitosamente")
						window.open('factura-'+data);
						load('vst-facturacion-admin','','#datofact');
					}
				})
			},
