...
<?php $row = $mcompra->listacli(); ?>
<form action="" class="op0">
	<div class="form-group">
		<label for="" class="label control-label col-sm-12 bolder">Buscar Proveedor de Compra:</label>
		<div class="col-sm-9">
			...
			submitHandler: function (form) {
				$.post('prc-ccompra-buscar',$(formulario).serialize(),function(data){
					if(data.trim()=='no') {
						...
						$.post('prc-mcompra-insertcompra_temp',0,function(data1){
							if(!isNaN(data1)) {
								load('vst-compra-adminproduc','buscaractivo=1&fact='+data1.trim(),'.compra');
							}
						})
					}
				})
