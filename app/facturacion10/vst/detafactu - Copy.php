...
<?php $rowdf = $mfacturacion->listadetafactura($_SESSION['s_factura']); ?>
			...
			<tbody>
				<?php foreach($rowdf as $rdf): ?>
					<tr>
						...
						<td>
							<div class="btn-group">
								<button class="btn btn-success btn-xs" title="Actualizar" onclick="modal('vst-facturacion-updatedetafac','ide=<?php echo $rdf->factdetatemp_ide; ?>')">
									<i class="fa fa-edit"></i>
								</button>
								<button class="btn btn-danger btn-xs" title="Borrar" onclick="modal('vst-facturacion-deletedetafac','ide=<?php echo $rdf->factdetatemp_ide; ?>')">
					...
					<tr>
						...
						<td align="center">
							<div class="btn-group">
								...
								<br><br>
								<button class="btn btn-danger btn-xs" title="Cancelar Venta" onclick="load('vst-facturacion-admin','0','0')">
		...
		$(formulario).validate({
			...
			submitHandler: function (form) {
				$.post('prc-mfacturacion-insertfact',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						load('vst-facturacion-admin','','');
						if(confirm('Factura procesada correctamente.\n¿Desea generar nueva factura?')==true) {
							load('vst-facturacion-admin','','');
						}
					}
				})
