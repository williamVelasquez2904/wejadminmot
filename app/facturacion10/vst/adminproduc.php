			...
			submitHandler: function (form) {
				...
				$.post('prc-cproducto-buscarfactu',$(formulario).serialize(),function(data){
					if(data.trim()=='no') {
						...
						load('vst-facturacion10-adminproduc','buscaractivo=1&fact=<?php echo $_SESSION['s_factura']; ?>','.factura');
					} else {
						load('vst-facturacion10-adminproduc','produc_ide='+data.trim()+'&buscaractivo=0','.factura');
						load('vst-facturacion10-detafactu','','.detafactu');
					}
				})
				<?php } else {?>
				$.post('prc-mfacturacion10-inserttemp',$(formulario).serialize(),function(data){
					if(!isNaN(data)) {
						load('vst-facturacion10-adminproduc','buscaractivo=1&fact=<?php echo $s_factura; ?>','.factura');
						load('vst-facturacion10-detafactu','','.detafactu');
					} else {
