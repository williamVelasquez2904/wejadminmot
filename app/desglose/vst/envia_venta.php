...
if (count($row) > 0) {
	...
	$mensaje_tipo = $row[0]->tipvta_descrip;
	$valor_tipo = $row[0]->desgvta_tipo;
...
$text_origen=Funciones::descrip_origen($origen_ide);

$titulo="[envia_venta.php] (11-09-2024). Venta. ORIGEN: $text_origen "."- Id. de la venta [$ide] ";
//$titulo="Venta en Desglose.  Origen: $text_origen "."Id. de la venta [$ide] ";

//echo //$fn->modalHeader('[envia_venta.php]-300824 Venta en desglose.  Id. de la venta [ '.$ide.'] ') 
	...
	<form action="" class="enviaventa">
		...
		<div class="form-group col-sm-2 col-xs-12">
			<label for="" class="label control-label col-sm-12 col-xs-12 bolder">% Asignado</label>
				<div class="col-sm-12 col-xs-12">
					<input type="text" class="form-control" name="f_porc_asig" id="f_porc_asig" value="<?php echo $row[0]->desgvta_porc_asig ;  ?>"  disabled>		
				</div>
		</div>
		<input type="hidden" class="form-control" name="porc_asig" id="porc_asig" value="<?php echo $row[0]->desgvta_porc_asig;  ?>" >		
...
<!-- 
		...
		<div class="form-group col-sm-2 col-xs-12">
			...
			<div class="col-sm-12 col-xs-12">
				<input type="text" class="form-control" name="f_tipo" id="f_tipo"  value="<?php echo $mensaje_tipo; ?>" disabled>
			</div>
		</div>
		<input type="hidden" class="form-control" name="tipoventa" id="tipoventa" value="<?php echo $valor_tipo; ?>">
